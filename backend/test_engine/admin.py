...
class CandidateAdmin(admin.ModelAdmin):
    list_display = ['id', 'name', 'email', 'phone', 'secret_code_1', 'secret_code_2']
    search_fields = ['email']

@admin.register(Response)
class ResponseAdmin(admin.ModelAdmin):
    list_display = ("id", "candidate", "test", "question", "answer")
    #list_filter = ("test", "candidate", "question__category", "question__difficulty", "marked_for_review")
    search_fields = ("candidate__name", "test__name", "question__text")
    def get_queryset(self, request):
        qs = super().get_queryset(request)
        return qs.exclude(answer="")  # Optional filter logic
...
class ScoreReportAdmin(admin.ModelAdmin):
    list_display = [
        'candidate', 'test', 'attempt_number', 'score',
        'max_possible', 'percentage',
        'total_correct', 'total_wrong', 'total_unattempted',
        'created_at'
    ]
    #list_filter = ("test","candidate")
    search_fields = ("candidate__name", "test__name")
    ...
    @admin.action(description="Generate Excel Score Report")
    def export_score_excel(self, request, queryset):
        messages = []
        ...
        for report in queryset:
            ...
            result = calculate_score_for_candidate(test, candidate, attempt_number=attempt)
...
class ArchivedResponseAdmin(admin.ModelAdmin):
    list_display = ["candidate", "question", "test", "attempt_number", "archived_at"]
    #list_filter = ["test", "attempt_number", "candidate", "question__category"]
    search_fields = ["candidate__name", "question__text"]



@admin.register(TestAssignment)
class TestAssignmentAdmin(admin.ModelAdmin):
    list_display = ["id", "candidate", "candidate__id","test", "valid_from", "valid_to", "max_attempts"]
    list_filter = ["test", "candidate"]
    search_fields = ["candidate__name", "candidate__email", "test__name"]


@admin.register(CandidateTestSession)
class CandidateTestSessionAdmin(admin.ModelAdmin):
    list_display = ["assignment", "attempt_number", "started_at", "completed", "current_section"]
    list_filter = ["assignment__test", "completed"]
...
class SectionStatusAdmin(admin.ModelAdmin):
    list_display = ["session", "section", "started_at", "submitted_at", "auto_submitted"]
    list_filter = ["section__category"]
    search_fields = ["session__assignment__candidate__name"]
